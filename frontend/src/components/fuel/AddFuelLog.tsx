// src/components/fuel/AddFuelLog.tsx — log a fill-up (sub-screen 3).
...
import { ArrowLeft, MapPin, Gauge, Clock, Cpu } from 'lucide-react-native';
...
export default function AddFuelLog({ userFuel, onCancel, onSaved }: Props) {
  ...
  // Load price + last odometer (from most recent log, else vehicle) + vehicle efficiency.
  useEffect(() => {
    (async () => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      Alert.alert('Saved', 'Your fill-up has been logged.');
      onSaved();
    } catch (e: any) {
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexGrow: 0, marginBottom: 18 }}>
            {FUEL_ORDER.map((f) => {
              ...
              return (
                <Pressable key={f} onPress={() => setFuelType(f)} style={[styles.chip, active ? { backgroundColor: m.color, borderColor: m.color } : null]}>
          ...
          <Input leftIcon={<MapPin size={18} color={TEXT_LIGHT} />} value={station} onChangeText={setStation} placeholder="e.g. Petronas Skudai" />
          ...
          <Pressable style={styles.dateField} onPress={() => setDateModal(true)}>
          ...
          <View style={styles.toggleRow}>
            ...
            <Switch value={fullTank} onValueChange={setFullTank} trackColor={{ false: BORDER, true: FP_PRIMARY }} thumbColor={CARD} />
          </View>
          <View style={styles.toggleRow}>
            <View style={{ flex: 1 }}>
            ...
            <Switch value={missedPrevious} onValueChange={setMissedPrevious} trackColor={{ false: BORDER, true: FP_PRIMARY }} thumbColor={CARD} />
          </View>

          <Text style={styles.label}>Notes (optional)</Text>
          <Input value={notes} onChangeText={setNotes} placeholder="Anything to remember" />
          ...
          <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()}>
            ...
            <View style={styles.dayRow}>
              {DAY_OPTS.map((d) => {
                ...
                return (
                  <Pressable key={d.label} onPress={() => setDay(d.offset)} style={[styles.dayChip, active && styles.dayChipActive]}>
            ...
            <ScrollView contentContainerStyle={styles.slotWrap} showsVerticalScrollIndicator={false}>
              {TIME_SLOTS.map((t) => {
                ...
                return (
                  <Pressable key={t} onPress={() => setTime(t)} style={[styles.slot, active && styles.slotActive]}>
                    <Text style={[styles.slotText, active && { color: CARD }]}>{t}</Text>
                  </Pressable>
                );
              })}
            </ScrollView>
            <Pressable style={styles.doneBtn} onPress={() => setDateModal(false)}>
...
const styles = StyleSheet.create({
  ...
  sheet: { backgroundColor: CARD, borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingTop: 10, paddingBottom: 24, maxHeight: '70%' },
